...
function ShowTable(Event) {
	if(QuizLock)return;
	//console.log(Event)
    sampleTable.Place = Event.currentTarget.Place;
    sampleTable.getElementsByTagName('th')[0].textContent = PlaceName(sampleTable.Place);
    sampleTable.style.display = 'table';
    sampleTable.style.left = Event.clientX + 'px';
    sampleTable.style.top = Event.clientY + 'px';
    Event.stopPropagation();
}

function dealSelection(Event) {
	if(QuizLock)return;
    var selPack = +Event.currentTarget.getAttribute('data-级别');
    sampleTable.Place.等级 = selPack;
    sampleTable.Place.图形.attr({fill : ColorList[selPack]});
...
function ShowQuiz(Event){
	if(QuizLock)return;
	//console.log(Event)
	Place = Event.currentTarget.Place;
	var depName = Place.Name.zh;
	if(confirm("想要打卡整个"+depName+"？那得考考你一些武大冷知识了！")==false)return;
	var depId = depList[depName];
	$.ajax({
		type: "GET",//方法类型
		dataType: "json",
		url: "/getQuestion" ,
...
var ColorList = ['#DDD','#cc9', '#cc3', '#9c9', '#693'];

function PlaceName(Place) {
    return Place.Name.hasOwnProperty(Language) ? Place.Name[Language] : Place.Name.zh;
}

function calc() {
    var sum = 0;
    for (var PlaceName in WHUSCHOOL) {
        sum += WHUSCHOOL[PlaceName].等级;
    }
    return sum;
}
...
window.onload = function () {
    ...
    var WHUmap = new Raphael('WHUmap');
    for (var Key in WHUSCHOOL) {
        var Place = WHUSCHOOL[Key];
        var 图形 = WHUmap.path(Place.路径);
        图形.attr({fill : ColorList[0]});
        Place.图形 = 图形;
        Place.等级 = 0;

        var x = 图形.getBBox().x + 图形.getBBox().width / 2 + Place.偏移.x;
        var y = 图形.getBBox().y + 图形.getBBox().height / 2 + Place.偏移.y;
        var 文字 = WHUmap.text(x, y, PlaceName(Place));

        [图形, 文字].forEach(
            function (元素) {
                元素.attr({cursor : 'pointer'});
                元素[0].onclick = ShowTable;
                元素[0].Place = Place;
				var depName = Place.Name.zh;
				if(depName=="文理学部"||depName=="信息学部"||depName=="工学部"||depName=="医学部"){
					元素[0].onclick = ShowQuiz;
				}
            }
        );
    }
	
    Array.from(document.getElementsByClassName('selPack')).forEach(
        function (selPack) {
            selPack.style.backgroundColor = ColorList[selPack.getAttribute('data-级别')];
            selPack.onclick = dealSelection;
        }
    );
